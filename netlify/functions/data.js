...
export default async (req) => {
  ...
  if (req.method === "POST") {
    ...
    try {
      // Netlify Blobs does not provide compare-and-swap writes. Use a strongly
      // consistent store, apply this idempotent mutation to the latest state,
      // and write the resulting snapshot. processedMutations prevents duplicate
      // client retries from applying the same change twice.
      const current = migrateState(await store.get(STATE_KEY, { type: "json" }));
      ...
      console.error("Oakdene mutation failed", error);
      return responseJSON({ error: "Could not save this change. Please try again." }, 503);
    }
  }
